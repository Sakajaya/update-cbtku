    ...
    <div class="row mb-3">
      <div class="col-md-3">
        <select id="filterKelas" class="form-select">
          ...
          <?php foreach ($availableClasses as $kelas): ?>
            <option value="<?= esc($kelas) ?>"><?= esc($kelas) ?></option>
          <?php endforeach; ?>
        </select>
      </div>
      <div class="col-md-3">
        <select id="filterRuang" class="form-select">
          <option value="">-- Semua Ruang --</option>
          <?php foreach ($availableRooms as $ruang): ?>
            <option value="<?= esc($ruang) ?>"><?= esc($ruang) ?></option>
          <?php endforeach; ?>
        </select>
      </div>
    </div>
        ...
        <tbody>
          <?php foreach ($sessions as $i => $s): ?>
            <tr class="row-student" data-id="<?= $s['id'] ?>">
              ...
              <td class="text-center"><?= esc($s['room'] ?? '-') ?></td>
              <td class="text-center">
...
<script>
  $(function () {
    const table = $('#tableAktivitasUser').DataTable({
      ...
      pageLength: 25,
      columnDefs: [
        { orderable: false, targets: [0, 12] } // Checkbox & Action non-sortable
      ],
      language: {
        search: "Cari:",
    ...
    $('#filterKelas').on('change', function() {
      const val = $(this).val();
      // Column 4 is Kelas (index 4)
      table.column(4).search(val ? '^' + $.fn.dataTable.util.escapeRegex(val) + '$' : '', true, false).draw();
    });

    $('#filterRuang').on('change', function() {
      const val = $(this).val();
      // Column 5 is Ruang (index 5)
    ...
    $(document).on('click', '.btn-reset-login', function () {
      ...
      Swal.fire({
        title: 'Buka Blokir & Lanjut Ujian?',
        html: `Apakah Anda yakin ingin mereset sesi login dan mengaktifkan kembali ujian untuk <b>${studentName}</b>?<br><small class='text-muted'>Siswa dapat login kembali dan <b>melanjutkan progres</b> tanpa kehilangan jawaban yang sudah ada.</small>`,
        icon: 'question',
        showCancelButton: true,
        confirmButtonColor: '#ffc107',
        cancelButtonColor: '#6c757d',
        confirmButtonText: 'Ya, Buka & Lanjut!',
        cancelButtonText: 'Batal'
      }).then((result) => {
        if (result.isConfirmed) {
          const payload = getCsrfData();
          payload.test_id = "<?= $test['id'] ?>"; // Kirim test_id agar sesi bisa di-reset statusnya

          $.post(`<?= site_url('admin/cbt/aktivitas/resetLogin') ?>/${studentId}`, payload)
            .done((res) => {
