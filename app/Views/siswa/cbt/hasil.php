...
<div class="container py-4">
  ...
  <?php if (!$showAnswerKey): ?>
    <div class="alert alert-warning d-flex align-items-center gap-2 mb-4" role="alert">
      <i class="bi bi-lock-fill fs-5"></i>
      <div>
        <strong>Kunci jawaban belum tersedia.</strong>
        Masih ada <strong><?= $activeCount ?></strong> siswa yang sedang mengerjakan ujian ini.
        Kunci jawaban akan ditampilkan setelah semua siswa selesai atau waktu ujian berakhir.
      </div>
    </div>
  <?php endif; ?>
    ...
    <?php foreach ($details as $no => $q): ?>
      <div class="card question-card">
        <div class="card-body">
          ...
          <?php if (in_array($q['type'], ['pg', 'pilihan_ganda', 'pg_kompleks', 'pgk'])): ?>
            <ul class="option-list">
              <?php foreach ($q['options'] as $key => $optText):
                ...
                $classes = [];
                if ($isStudent) $classes[] = 'student-answer';
                if ($showAnswerKey) {
                    if ($isCorrect) $classes[] = 'correct';
                    elseif ($isStudent && !$isCorrect) $classes[] = 'incorrect';
                }
                ?>
            ...
            <div class="answer-info">
              ...
              <?php if ($showAnswerKey): ?>
                ...
                <?php
                $qScore  = (float) ($q['score'] ?? 0);
                $isPGK   = in_array($q['type'], ['pg_kompleks', 'pgk']);
                if ($qScore >= 1.0) {
                    echo '<span class="correct">✅ Benar</span>';
                } elseif ($qScore > 0) {
                    echo '<span class="text-warning fw-bold">⚠️ Sebagian Benar (' . round($qScore * 100) . '%)</span>';
                } else {
          ...
          <?php elseif (in_array($q['type'], ['benar_salah', 'bs'])): ?>
              ...
              <table class="table table-bordered table-sm mt-2">
                ...
                <tbody>
                  ...
                  foreach ($rows as $idx => $rowKey):
                    if (empty($q['options'][$rowKey])) continue;
                    $totalItems++;
                    $stdChar = $stdAns[$idx] ?? '';
                    $keyChar = $keyAns[$idx] ?? '';
                    $isMatch = ($showAnswerKey && $stdChar === $keyChar && $stdChar !== '');
                    if ($isMatch) $matches++;
                    ?>
                    <tr>
                      ...
                      <?php if ($showAnswerKey): ?>
                        ...
                        <td class="text-center"><?= $isMatch ? '✅' : '❌' ?></td>
                      <?php endif; ?>
                    </tr>
                  <?php endforeach; ?>
                </tbody>
            ...
            <?php if ($showAnswerKey): ?>
              <div class="answer-info">
                ...
                if ($scoreBS >= 1)      echo '<span class="correct">✅ Benar Semua</span>';
                elseif ($scoreBS > 0)   echo '<span class="text-warning fw-bold">⚠️ Sebagian Benar (' . round($scoreBS * 100) . '%)</span>';
                else                    echo '<span class="wrong">❌ Salah Semua</span>';
                ?>
              </div>
            <?php endif; ?>
